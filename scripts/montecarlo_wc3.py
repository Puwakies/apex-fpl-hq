...
def S_div_bank6(gw):  return (apply_swap(DIV) if gw>=6 else DIV), (gw==7)   # MAIN plan
def S_orig_bank6(gw): return (apply_swap(ORIG) if gw>=6 else ORIG), (gw==7)
def S_div_swap4(gw):  return (apply_swap(DIV) if gw>=4 else DIV), (gw==7)
def S_div_noswap(gw): return DIV, (gw==7)
STRATS={
 "DIV bank->swap6 +TC7":  S_div_bank6,
 "ORIG bank->swap6 +TC7": S_orig_bank6,
 "DIV swap@GW4 +TC7":     S_div_swap4,
 "DIV no-swap +TC7":      S_div_noswap,
}
N=6000
